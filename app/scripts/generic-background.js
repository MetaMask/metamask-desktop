...
export async function initialize({ localStore: localStorage, triggerUi, browser, notificationManager }) {
    ...
    const initState = await loadStateFromPersistence(localStore);
    const initLangCode = await getFirstPreferredLangCode(browser);
    await setupController({ initState, initLangCode, openPopup, triggerUi });
 ...
 */
async function loadStateFromPersistence(localStore) {
    // migrations
 ...
 */
function setupController({ initState, initLangCode, openPopup, triggerUi, browser }) {
    //
    // MetaMask Controller
    //
