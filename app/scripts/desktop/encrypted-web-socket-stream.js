...
export default class EncryptedWebSocketStream extends Duplex {
    ...
    async _onMessage (data) {
        if(!this._targetPublicKey) {
            if(data.publicKey) {
                ...
                log.debug('Ignoring message as waiting for public key', flattenMessage(data));
            }
    ...
    async _write(msg, encoding, cb) {
        if(!this._targetPublicKey) {
            log.debug('Skipping sending message as waiting for public key', flattenMessage(msg));
            cb();
            return;
        }
